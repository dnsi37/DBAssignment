...
import kr.ac.inha.sample.login.mapper.LoginMapper;
...
public class LoginServiceImpl implements LoginService{
	...
	public String saveRegisterInfo(RegistDto mbr) throws Exception {
		...
		try {
			String htmlStr = "<h2>"+members.getLastName() + members.getFirstName()+"님 안녕하세요.</h2>" 
					...
					+ "<a href='http://192.168.0.8:8080/login/certification.do?userNo="+members.getUserNo()+"&userId="+ members.getUserId() +"&userKey=18f198e0-92a9-447c-9adf-b93b4b578644'>[인증하기]</a></p>"
	...
	public String checkUser(MembersDto mbr) throws Exception {
		...
		MembersDto members = membersList.get(0);
		if (!members.getUserPass().equals(mbr.getUserPass())) {
